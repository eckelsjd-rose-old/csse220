...
public class TeamGradebook {
	...
	private String handleGetAbsences(String studentName) {
		...
		int sampleAbsence = student.handleGetAbsences();
		return Integer.toString(sampleAbsence);
	}
	...
	private String handleAddTeam(String teamName, ArrayList<String> memberNames) {
		...
		for (String name : memberNames) {
			if (currentNames.contains(name)) {
				currentStudents.add(this.getStudentByName(name));
			} else {
		...
		newTeam.addStudents(currentStudents);
		newTeam.addStudents(newStudents);
	...
	private String handleGetAverage(String studentName) {
		...
		for (Student student : this.students) {
			if (student.getName().equals(studentName)) {
				long average = Math.round(student.getAverage());
	...
	// Did not work with a partner on this assignment . -Josh Eckels
	private String handleGetBestTeam() {
		// TODO: Your code here
		double bestAverage = 0.0;
		String bestTeam = "";
		HashMap<String, Double> teamMap = new HashMap<>();

		for (Team team : this.teams) {
			teamMap.put(team.getName(), team.getTeamAverage());
		}

		for (String teamName : teamMap.keySet()) {
			if (teamMap.get(teamName) > bestAverage) {
				bestAverage = teamMap.get(teamName);
				bestTeam = teamName;
			}
		}
		return bestTeam;
