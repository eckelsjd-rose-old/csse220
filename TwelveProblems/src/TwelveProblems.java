...
public class TwelveProblems {
	...
	public static char mostCommonCharacter(String input) {
		...
		for (int i=0; i < input.length(); i++) {
			...
			for (int k=0; k < input.length(); k++) {
				if (current == input.charAt(k)){
					count = count + 1;
				}
			}
			if (count > greatest) {
				greatest = count;
				common = input.charAt(i);
			}
			count = 0;
		}
		return common;
	...
	public static int[] powersOfTwo(int maxExponent) {
		...
		if (maxExponent < 0) {
			int[] myEmpty = new int[0];
			return myEmpty;
		}
		
		int[] myList = new int[maxExponent + 1];
		
		for (int i = 0; i < myList.length; i++) {
			myList[i] = (int) Math.pow(2, i);
			}
	...
	public static int timesOccur(int[] shorter, int[] longer) {
		...
		for (int j=0; j < longer.length - shorter.length + 1; j++) {
			if (longer[j] == shorter[start]) {
				for (int k=j; k < j + shorter.length; k++) {
					if (longer[k] == shorter[start++]) {
						count1 = count1 + 1;
					}
				}
				if (count1 == shorter.length) {
					count2 = count2 + 1;
				}
			}
			count1 = 0;
	...
	public static ArrayList<String> threeCharacterStrings(String input) {
		...
		for (int i=0; i < input.length() - 2; i++) {
			String subS = input.substring(i, i + 3);
			output.add(subS);
		}
